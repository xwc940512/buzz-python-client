...
class MainTest(unittest.TestCase):
  ...
  def setUp(self):
    self.oauth_config = yaml.load(open(CONFIG_PATH).read())
    self.oauth_consumer_key = self.oauth_config['oauth_consumer_key']
    self.oauth_consumer_secret = self.oauth_config['oauth_consumer_secret']
    self.oauth_token_key = self.oauth_config['oauth_token_key']
    self.oauth_token_secret = self.oauth_config['oauth_token_secret']

    self.buzz_testing_id = self.oauth_config['testing_id']
    self.buzz_testing_account = self.oauth_config['testing_account']
    self.buzz_target_id = self.oauth_config['target_id']
    self.buzz_target_account = self.oauth_config['target_account']
    self.buzz_post_id = self.oauth_config['post_id']

    self.client = buzz.Client()
    self.client.build_oauth_consumer(self.oauth_consumer_key, self.oauth_consumer_secret)
    self.client.build_oauth_access_token(self.oauth_token_key, self.oauth_token_secret)
  ...
  def test_registered_consumer(self):
    client = buzz.Client()
    client.build_oauth_consumer(self.oauth_consumer_key, self.oauth_consumer_secret)
    client.oauth_scopes.append(buzz.FULL_ACCESS_SCOPE)
    request_token = \
      client.fetch_oauth_request_token('http://example.com/callback/')
    assert request_token.key, "Request token key missing."
  ...
  def test_unfollow(self):
    ...
    time.sleep(0.3)
    followers = self.client.followers(self.buzz_target_id).data
    assert not self.buzz_testing_id in [follower.id for follower in followers], \
      "Should not have been able to find the account in followers list."
    following = self.client.following(self.buzz_testing_id).data
    assert not self.buzz_target_id in [followee.id for followee in following], \
...
if __name__ == '__main__':
  CONFIG_PATH = os.environ.get('BUZZ_TEST_CONFIG', 'examples/test_config.yaml')
